...
import (
	...
	revisioncommentsapi "github.com/lorehub/lorehub/services/api/internal/revisioncomments"
...
func (api *API) registerCoreRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /api/v1/actions/runner/claim", api.runnerClaim)
	mux.HandleFunc("POST /api/v1/actions/runner/jobs/{jobID}/heartbeat", api.runnerJobHeartbeat)
	mux.HandleFunc("GET /api/v1/actions/runner/jobs/{jobID}/cancellation", api.runnerJobCancellation)
	mux.HandleFunc("GET /api/v1/actions/runner/jobs/{jobID}/context", api.runnerJobContext)
	mux.HandleFunc("POST /api/v1/actions/runner/jobs/{jobID}/token", api.runnerJobToken)
	mux.HandleFunc("POST /api/v1/actions/runner/jobs/{jobID}/logs", api.runnerJobLogs)
	mux.HandleFunc("POST /api/v1/actions/runner/jobs/{jobID}/artifacts", api.runnerJobArtifacts)
	mux.HandleFunc("POST /api/v1/actions/runner/jobs/{jobID}/complete", api.runnerJobComplete)
		...
		if codeClient, ok := api.lore.(loreclient.CodeClient); ok {
			...
			if comments, ok := api.collabStore.(collab.RevisionCommentStore); ok {
				revisioncommentsapi.Register(
					mux, comments, api.collabStore, api, codeClient, api.loreCredentials,
					api.serviceSubjects.PublicReader, api.logger,
				)
			}
