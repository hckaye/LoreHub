...
func (store *Store) Search(
	...
	if kind == "all" || kind == "repositories" {
		results.Repositories, err = store.searchRepositories(ctx, query, viewerID, limit)
		if err != nil {
			return SearchResults{}, err
		}
	}
	if kind == "all" || kind == "organizations" {
		results.Organizations, err = store.searchOrganizations(ctx, query, viewerID, limit)
		if err != nil {
			return SearchResults{}, err
		}
	}
	if kind == "all" || kind == "users" {
		results.Users, err = store.searchUsers(ctx, query, limit)
		if err != nil {
			return SearchResults{}, err
		}
	}
...
func (store *Store) searchRepositories(
	ctx context.Context,
	query string,
	viewerID string,
	limit int,
...
func (store *Store) searchOrganizations(
	ctx context.Context,
	query string,
	viewerID string,
	limit int,
	...
	sqlQuery := `
		...
		WHERE (
		    to_tsvector(
		        'simple'::regconfig,
		        COALESCE(o.slug, '') || ' ' || COALESCE(o.display_name, '') || ' ' || COALESCE(o.description, '')
		    )
		        @@ plainto_tsquery('simple', $1)
...
func (store *Store) searchUsers(ctx context.Context, query string, limit int) ([]UserSearchResult, error) {
		...
		WHERE status = 'active'
		  AND (
		      to_tsvector(
		          ...
		          @@ plainto_tsquery('simple', $1)
		      OR lower(concat_ws(' ', username, display_name, bio, company, location))
		          LIKE '%' || lower($1) || '%'
		  )
