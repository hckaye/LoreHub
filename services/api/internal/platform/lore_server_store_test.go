...
func TestLoreServerRegistrationTokenConsumptionIsAtomic(t *testing.T) {
	...
	if _, err := store.CreateRegistrationToken(ctx, owner, organizationSlug, input); err != nil {
	...
	for attempt := range errorsByAttempt {
		...
		go func(index int) {
			defer wait.Done()
			<-start
			_, errorsByAttempt[index] = store.ConsumeRegistrationToken(ctx, digest, time.Now().UTC())
		}(attempt)
	}
	close(start)
	wait.Wait()
...
func TestLoreServerRegistrationAuthenticationHeartbeatAndRevocation(t *testing.T) {
	...
	if _, err := store.CreateRegistrationToken(ctx, owner, organizationSlug,
