...
const maxRequestBytes = 6_100_000
...
func (api *API) storeError(
	...
	case errors.Is(err, platform.ErrInvalidInput), errors.Is(err, errInvalidInput):
		writeProblem(writer, http.StatusBadRequest, "invalid_input", "Revision comment fields are invalid")
	default:
		if api.logger != nil {
