...
const maxRequestBytes = 6*maxDiscussionBodyBytes + 64*1024
...
func (api *API) storeError(
	...
	case errors.Is(err, platform.ErrConflict):
		writeProblem(writer, http.StatusConflict, "conflict", "The discussion cannot be changed in its current state")
