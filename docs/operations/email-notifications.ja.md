...
内蔵アカウントのパスワード再設定メールは、同じ`LOREHUB_SMTP_*`の設定を使い、
`LOREHUB_NOTIFICATION_EMAIL_ENABLED=true`のあいだ利用できます。SMTPが未設定の場合、ログイン画面は再設定の
入り口を表示しません。任意のKeycloakプロファイルでログインを行う場合、アカウント確認とパスワード再設定の
メールにはKeycloak側のSMTP設定を使います。設定方法は
