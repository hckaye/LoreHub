...
利用者へ直接アクセスを付与するときは、7日間有効な招待を送ります。招待された利用者がアカウント設定から承諾するまで、
リポジトリの権限は付与されません。承諾または辞退できるのは招待された本人だけです。リポジトリ管理者は承諾待ちの招待を
取り消せます。既存の利用者に対する役割の変更はすぐに反映され、アクセスを削除すると直ちに利用できなくなります。
...
招待、役割、team、collaborator、方針、Link、obliterate、作成処理の変更は、監査記録とoutboxを同じPostgreSQL
transactionで保存します。どちらか一方だけが成功することはありません。
