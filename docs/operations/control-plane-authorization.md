...
Composeのrunnerも`LOREHUB_ENV`を継承します。開発時だけ`LOREHUB_RUNNER_AUTH_MODE=disabled`を使えます。本番でrunnerを
起動する場合は`LOREHUB_RUNNER_AUTH_MODE=bearer`、HTTPSのOIDC issuer/audience、managed root domain、署名鍵、JWKS、
AuthURL、TLS設定を本番値で与えます。設定が足りないrunnerは起動せず、CI checkoutも専用service principalの短命tokenを
発行できない限り失敗します。
